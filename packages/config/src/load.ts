...
function shapeFromEnv(env: EnvSource) {
  ...
  return {
    ...
    session: compact({
      ...
      idleTimeoutSeconds: env.PAS_SESSION_IDLE_TIMEOUT_SECONDS,
    ...
    authentication: compact({
      ...
      passwordHashCostLog2: env.PAS_AUTH_PASSWORD_HASH_COST_LOG2,
      passwordHashBlockSize: env.PAS_AUTH_PASSWORD_HASH_BLOCK_SIZE,
      passwordHashParallelism: env.PAS_AUTH_PASSWORD_HASH_PARALLELISM,
    }),
    encryption: compact({
